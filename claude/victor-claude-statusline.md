...
**Yellow means aimed here.** The one fact the relay publishes — this is the
session its words would come to — on the one row that is always on screen. Black
on the 226: yellow that bright cannot carry white text, and a badge that has to
be squinted at is the failure the background was added to fix.

The marker is one line with one word — `ttys006` — read with a single
`read -r _bt`.
...
Every epoch-to-clock conversion goes through one helper that tries BSD `date -r`
first and GNU `date -d @` second, so the same file runs on macOS and on Linux.
The clock falls back to a short sleep countdown glued to `💤` if neither
spelling can resolve the wake epoch. Near the window reset, the gate may use
